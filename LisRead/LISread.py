...
"""
    ...
    for k in range(1,n+1):
        ...
        if noExtrema:
            ...
            m = re.match(r"\s+(?P<num>[0-9]+)\s+:\s+(?P<bts>([0-9]+\s+[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s+)+)"\
                ".+"\
                "Variable extrema :\s+(?P<extrema>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
                "Times of maxima :\s+(?P<textrema>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
                , w, re.MULTILINE|re.DOTALL)        
        else:
            m = re.match(r"\s+(?P<num>[0-9]+)\s+:\s+(?P<bts>([0-9]+\s+[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s+)+)"\
                ".+"\
        ...
        if onlyExtrema:
            ...
            res['vext'].append( [float(x) for x in m.group('extrema').replace(lineSplit,'').split()])
            res['text'].append( [float(x) for x in m.group('textrema').replace(lineSplit,'').split()])
        elif noExtrema:
