...
def read(fname, onlyExtrema=True):
...
"""
    ...
    #
    f = rs[0].split('\r\n') # separar la primer seccion renglon-por-renglon
    ...
    nrem = nv
    while nrem > 0:
        ll = zip(f[kk][23:].split(), f[kk+1][23:].split())
        nvars.extend(ll)
    ...
    for k in range(1,n+1):
        ...
        res['vext'].append( map(float,m.group('extrema').replace('\r\n','').split()) )
        res['text'].append( map(float,m.group('textrema').replace('\r\n','').split()) )
        if not onlyExtrema:
            res['vmax'].append( map(float,m.group('vmax').replace('\r\n','').split()) )
            res['tmax'].append( map(float,m.group('tmax').replace('\r\n','').split()) )
            res['vmin'].append( map(float,m.group('vmin').replace('\r\n','').split()) )
            res['tmin'].append( map(float,m.group('tmin').replace('\r\n','').split()) )
        
        # procesar la lista de interruptores y sus tiempos de actuacion
        bts = m.group('bts').replace('\r\n','').split()
...
if __name__ == '__main__':
    r = read('FluxMax-1000ptsm.LIS',onlyExtrema=True)
