...
#
# convertir a EMF, pasando previamente por SVG
#
def toEmf(fig,emf_file, annex=None):
    # path al InkScape, para convertir de SVG a EMF y mantener la calidad de los graficos.
    inkPath = r"C:\Program Files (x86)\Inkscape\inkscape.exe"
    fig.savefig("%s.svg"%(emf_file,))
    if annex:
        prog = subprocess.call(r"%(ink)s -z %(name)s.svg -M %(name)s_%(annex)s.emf"%{'ink':inkPath, 'name':emf_file, 'annex':annex}, shell=False)
    else:
        prog = subprocess.call(r"%(ink)s -z %(name)s.svg -M %(name)s.emf"%{'ink':inkPath, 'name':emf_file}, shell=False)
    os.system('del "%s.svg"'%(emf_file,))



def read_det(fname,lineSplit='\n'):
...
Si noExtrema es True, ignora onlyExtrema y procesa los valores vmax y vmin
"""
    #
    ...
    #
    rs = re.split("Extrema of output variables follow.   Order and column positioning are the same as for the preceding time-step loop output.", r)#re.split("Random switching times for simulation number", r)
    ...
    for k in range(n1+1,n2):
        if not atp[k].startswith('C'):
            ...
            t_sw_1 = atp[k][14:24].strip()
            t_sw_2 = atp[k][24:35].strip()
            switches[ns] = (f, t, t_sw_1, t_sw_2) # from - to
    ...
    # o Branch energy
    nenerg = 0
    if f[kk].startswith('Branch energy'):
    ...
    while nrem > 0:
        l1 = []
        for i in range(18,len(f[kk]),11):
            l1.append(f[kk][i:i+11].strip())

        l2 = []
        for i in range(18,len(f[kk+1]),11):
            l2.append(f[kk+1][i:i+11].strip())
        
        # se corrige que hace cuando l2 queda vacio (todavia queda detectar si len(l2) < len(l1)
        # pero nunca se dio el caso.
        if l2:
            if len(l1) == len(l2):
                ll = list(zip(l1, l2))
            else:
                # l2 es mas corta que l1
                ll = list(zip(l1, l2))
                ll.extend(list(zip(l1[len(l2):],['']*(len(l1)-len(l2)))))
        else:
            ll = list(zip(l1, ['']*len(l1))) # completa con un l2 lleno de ['','','',...,'']
    ...
    # Nota: se mejoro la deteccion de numeros con el pattern '[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?'
    # lo que hace que hasta la ultima corrida sea detectada correctamente.
    #
    res = {'vmax':[], 'vmin':[], 'tmax':[], 'tmin':[]}
    
    rs = re.split('%s%s'%(lineSplit,lineSplit),rs[1])[0]


    w = rs.replace('%s%s'%(lineSplit,lineSplit),'\nVariable extrema : ') # separa con dos \r\n consecutivos ....
    # obtener las listas de valores

    # m = re.match(r"\s+(?P<num>[0-9]+)\s+:\s+(?P<bts>([0-9]+\s+[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s+)+)"\
    #     ".+"\
    #     "Variable maxima :\s+(?P<vmax>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
    #     "Times of maxima :\s+(?P<tmax>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
    #     "Variable minima :\s+(?P<vmin>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
    #     "Times of minima :\s+(?P<tmin>([+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?\s*)+)"\
    ...
    res['vmax'].append( [float(x) for x in m.group('vmax').replace(lineSplit,'').split()])
    res['tmax'].append( [float(x) for x in m.group('tmax').replace(lineSplit,'').split()])
    res['vmin'].append( [float(x) for x in m.group('vmin').replace(lineSplit,'').split()])
    res['tmin'].append( [float(x) for x in m.group('tmin').replace(lineSplit,'').split()])
    ...
    # b = [int(x) for x in bts[0::2]]
    # t = [float(x) for x in bts[1::2]]
